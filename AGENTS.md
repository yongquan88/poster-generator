...
本项目服务于一家海报设计与印刷电商公司，是围绕该公司的真实业务流程定制的 AI 海报生成助手。公司主要承接生日海报、结婚海报、毕业海报，以及商铺开业、促销活动等商业海报设计与印刷需求。

项目是纯前端应用，构建产物部署为静态资源；本地开发和 Docker 部署可通过代理解决浏览器 CORS 问题。

## 业务背景与常用尺寸

代理在设计、文案、提示词、尺寸和编辑流程上都应优先贴合“海报设计与印刷电商”的业务场景。默认用户目标通常不是泛用图片创作，而是生成适合后期排版、印刷、交付客户的海报底图或成品参考图。
...
- 商铺开业海报
- 促销活动海报
- 其他线下门店宣传、活动宣传、商业喷绘海报

常用尺寸以项目中“海报”栏目配置为准，配置位置是 `src/lib/size.ts` 的 `POSTER_SIZE_PRESETS`。当前预设包括：

- `80×180cm`：`1712x3840`，4:9 竖版海报
- `60×160cm`：`1440x3840`，3:8 竖版海报
- `78×200cm`：`1504x3840`，39:100 竖版海报
- `98×200cm`：`1888x3840`，49:100 竖版海报
- `120×200cm`：`2208x3680`，3:5 竖版海报
- `60×90cm`：`2336x3504`，2:3 竖版海报
- `50×70cm`：`2400x3360`，5:7 竖版海报
- `45×100cm`：`1728x3840`，9:20 竖版海报
- `100×200cm`：`1920x3840`，1:2 竖版海报

涉及尺寸、默认参数、提示词、预设或 UI 文案时，不要随意发明新的常用尺寸；应先检查 `POSTER_SIZE_PRESETS`，并确保文档、UI 和测试保持一致。
